...
let comidasFavoritas = ['fideos con salsa', 'milanesa con papas fritas', 'arroz con pollo'];
...
numeros = [1, 2, 3]; //ERROR porque el array constante ya esta creado, no se volver a declarar
*/
...
/*
let notasCuatrimestre = [7, 6, 9, 4, 7];
let sumatoriaNotas = 0;

function calcularPromedio(notasCuatrimestre){
    let i;
    for(i = 0; i < notasCuatrimestre.length ; i++){
        sumatoriaNotas += notasCuatrimestre[i];

    }
    return sumatoriaNotas / notasCuatrimestre.length;
}

console.log('El promedio de notas es ' + calcularPromedio(notasCuatrimestre));
*/
